...
export default function Destination() {
  ...
  const openAdd  = () => { setEditingDest(null); setIsModalOpen(true); };
  const openEdit = (dest) => { setEditingDest(dest); setIsModalOpen(true); };
  const closeModal = () => { setIsModalOpen(false); setEditingDest(null); };
    ...
    <div className="destination-page">
      <div className="destination-card">
        ...
        <div className="destination-header">
          ...
          <button className="destination-add-btn" onClick={openAdd}>
            <Plus size={16} />
        ...
        <div className="destination-list">
          ...
          {!loading && !fetchError && paginated.map((dest, i) => (
            <div className="destination-row" key={dest.id ?? i}>
              <span className="dest-name">{dest.name}</span>
              <button className="edit-btn" onClick={() => openEdit(dest)}>
                <PencilLine size={14} />
              </button>
            </div>
          ))}
